...
export default class MessageHandler {
    ...
    static line(data) {
        let commands = {
            'start': 'move-to-first-character-of-line',
            'end': 'move-to-end-of-line',
            'screen-start': 'move-to-beginning-of-line'
        };

        MessageHandler._dispatch('editor:' + commands[data.position]);
    }
    ...
    static navigation(data) {
        ...
        atom.workspace.getActiveTextEditor()[commands[data.direction]](amount);
    }
    ...
    static word(data) {
        ...
        let commands = {
            'next': ['move-to-next-word-boundary', 'move-to-next-word-boundary'],
            'previous': ['move-to-previous-word-boundary', 'move-to-previous-word-boundary'],
            'start': ['move-to-beginning-of-word'],
            'end': ['move-to-end-of-word']
        };

        for (let command of commands[data.direction]) {
            for (let i = 0; i < quantifier; i++) {
                MessageHandler._dispatch('editor:' + command);
            }
        }
    }
}
