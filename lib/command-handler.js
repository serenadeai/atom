...
export default class CommandHandler {
    ...
    COMMAND_TYPE_COPY(data) {
        atom.clipboard.write(data.text_);
    }
    ...
    COMMAND_TYPE_DIFF(data) {
        ...
        atom.workspace.getActiveTextEditor().setText(data.source_);
    ...
    COMMAND_TYPE_NAVIGATION(data) {
        let multiplier = (data.page_) ? 25 : 1;
        let amount = multiplier;
        if (data.quantifier_) {
            amount = data.quantifier_ * multiplier;
        }

        let keycodes = [];
        for (let i = 0; i < amount; i++) {
            keycodes.push(data.direction_);
        }
    ...
    COMMAND_TYPE_PASTE(data) {
        ...
        editor.setText(source.substring(0, data.cursor_) + text + source.substring(data.cursor_));
        this._moveCursor(data);
    }
    ...
    COMMAND_TYPE_SEARCH(data) {
        ...
        Applescript.type(data.text_);
    }

    COMMAND_TYPE_SHOW_ALTERNATIVES(data) {
        this.delegate.showAlternatives(data.sequences);
    }
    ...
    COMMAND_TYPE_SPLIT(data) {
        this._dispatch('pane:split-' + data.direction_ + '-and-copy-active-item');
    }
    ...
    COMMAND_TYPE_SWITCH_TAB(data) {
        atom.workspace.getActivePane().activateItemAtIndex(data.index_ - 1);
    }
    ...
    COMMAND_TYPE_WINDOW(data) {
        ...
        this._dispatch('window:focus-pane-' + commands[data.direction_]);
    }
}
