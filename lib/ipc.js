...
export default class IPC {
    ...
    async handle(parseResponse) {
        ...
        if (parseResponse.execute) {
            // show auto-executed commands
            if (parseResponse.autoExecute && !this._internal(parseResponse.execute)) {
                this.delegate.showAlternatives([parseResponse.execute]);
                this.delegate.highlightAlternative(0);
            }
