...
export default {
    ...
    connect() {
        ...
        this.socket.on('message', (command) => {
    ...
    handleCommand(command) {
        // call the static method on the command handler of the same name
        let o = CommandHandler.prototype.constructor;
        if (!(command.type in o)) {
            return;
        }

        this.sendMessage(command.type + ':callback', command.id, o[command.type](command));
    },
